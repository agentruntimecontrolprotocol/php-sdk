...
// reviewer side
$reviewer->onPermissionRequest(function ($req) use ($reviewer) {
    $verdict = review($ticket, $req);
    // emit permission.grant or permission.deny
});
```

Two separate sessions. Same envelope contract. The reviewer's "no"
arrives at the generator as a structured `PermissionDeniedException`
with a `reason` field, not a 403 with a stack trace.
...
- `main.php` — bounded loop. Two sessions, one process for the demo.
- `agents.php` — `Patch`, `ReviewVerdict`, `propose`, `review` stubs.
