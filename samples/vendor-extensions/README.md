...
$audio = $client->invokeTool(EXT_DEMODULATE,
    ['iq_artifact_id' => $cap->value['artifact_id'], ...]);
```
...
- `main.php` — client side: tune, gain, capture, demodulate, then
...
uses an `__optional` argument key as the marker — adapt to whatever
your runtime registry expects.
