...
/**
 * Drives the `job.submit` lifecycle (ยง7.1): agent resolution,
...
final readonly class JobSubmitHandler
{
    ...
    public function handle(Session $session, Envelope $env, JobSubmit $msg): void
    {
        ...
        if (!$resolved instanceof ResolvedTool) {
            // ยง12: an unregistered agent is AGENT_NOT_AVAILABLE (#150).
        ...
        // slot with the request fingerprint (ยง7.2): the claim is released
        ...
        try {
            ...
            // LEASE_SUBSET_VIOLATION (widening overlay, ยง9.4), or another
    ...
    /**
     * Claim the ยง7.2 idempotency slot before acceptance. Returns true
    ...
    /**
     * Merge the ยง7.1 `lease_request` / `lease_constraints` payload fields
    ...
    /**
     * The ยง7.2 idempotency key: the spec carries it in the job.submit
    ...
    /**
     * Answer a duplicate submission under an existing claim (ยง7.2):
    ...
    private function answerDuplicate(
        ...
        if (!$accepted instanceof Envelope) {
            ...
            // can no longer be reproduced (ยง7.2).
            $this->emitSubmitError($session, $env, new ErrorPayload(
                'INTERNAL_ERROR',
    ...
    private function emitJobAccepted(
        ...
        // The message id was pre-minted for the ยง7.2 claim so an
    ...
    /**
     * Initial ยง9.6 budget counters for the job.accepted payload, when a
    ...
    /** ยง7.1: enforce `max_runtime_sec` by cancelling into TIMEOUT (ยง7.3). */
    ...
    private function completeJob(SubmitJobContextSpec $spec, mixed $value): void
    {
        ...
        if ($job->streamedResultId !== null) {
            ...
            if ($value !== null) {
                $this->failJob($spec, new ErrorPayload(
                    'INVALID_REQUEST',
                    'job mixed inline result and result_chunk (ยง8.4)',
                    false,
                ));
                return;
            }
            if (!$job->resultStreamClosed) {
                $this->failJob($spec, new ErrorPayload(
                    'INVALID_REQUEST',
                    'result_chunk stream not terminated: final chunk must carry more=false (ยง8.4)',
                    false,
                ));
                return;
            }
        ...
        // ยง7.3/ยง8.4: a single terminal job.result resolves the synchronous
        // submitter via correlation_id. It carries either the inline result
        // or the ยง8.4 result_id + result_size of the streamed result.
    ...
    /**
     * ยง7.2: duplicate submitters whose identical retry arrived while the
    ...
    /**
     * Record the terminal outcome on the ยง7.2 claim so a later identical
    ...
    {
        // ยง7.4: client cancellation terminates with job.error
    ...
    {
        // ยง7.3: max_runtime_sec expiry terminates with job.error
