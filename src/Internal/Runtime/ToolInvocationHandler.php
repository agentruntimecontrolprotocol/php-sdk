...
final class ToolInvocationHandler
{
    ...
    public function handle(Session $session, Envelope $env, ToolInvoke $msg): void
    {
        ...
        $job = $this->runtime->jobs->start($session, $env->id, $env->traceId, $msg->tool);
        ...
        $job->future = async(function () use ($session, $env, $msg, $job, $handler): void {
            $this->runHandler($session, $env, $msg, $job, $handler);
        });
    }
    ...
    private function runHandler(
        ...
        try {
            ...
            $this->failJob($session, $env, $job, ErrorPayload::fromException($e));
        } catch (\Throwable $e) {
            $this->failJob($session, $env, $job, new ErrorPayload('INTERNAL', $e->getMessage()));
        }
    }
    ...
    private function rememberIdempotent(Session $session, Envelope $env): void
    {
        ...
        $this->runtime->eventLog->rememberIdempotent(
            $principal,
            (string) $key,
            (string) $env->id,
